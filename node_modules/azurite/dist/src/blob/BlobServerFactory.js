    ...
    async createServer(blobEnvironment) {
        ...
        if (!isVSC) {
            ...
            if (isSQL) {
                ...
                const config = new SqlBlobConfiguration_1.default(env.blobHost(), env.blobPort(), env.blobKeepAliveTimeout(), databaseConnectionString, constants_1.DEFAULT_SQL_OPTIONS, constants_3.DEFAULT_BLOB_PERSISTENCE_ARRAY, !env.silent(), undefined, debugFilePath !== undefined, debugFilePath, env.loose(), env.skipApiVersionCheck(), env.cert(), env.key(), env.pwd(), env.oauth(), env.disableProductStyleUrl());
                return new SqlBlobServer_1.default(config);
            }
            else {
                const config = new BlobConfiguration_1.default(env.blobHost(), env.blobPort(), env.blobKeepAliveTimeout(), (0, path_1.join)(location, constants_3.DEFAULT_BLOB_LOKI_DB_PATH), (0, path_1.join)(location, constants_3.DEFAULT_BLOB_EXTENT_LOKI_DB_PATH), constants_3.DEFAULT_BLOB_PERSISTENCE_ARRAY, !env.silent(), undefined, debugFilePath !== undefined, debugFilePath, env.loose(), env.skipApiVersionCheck(), env.cert(), env.key(), env.pwd(), env.oauth(), env.disableProductStyleUrl(), env.inMemoryPersistence());
