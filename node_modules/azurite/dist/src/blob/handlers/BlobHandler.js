...
class BlobHandler extends BaseHandler_1.default {
    ...
    async downloadBlockBlobOrAppendBlob(options, context, blob) {
        ...
        try {
            ...
            rangesParts = (0, utils_2.deserializeRangeHeader)(context.request.getHeader("range"), context.request.getHeader("x-ms-range"));
        }
        catch (err) {
            this.logger.info(`BlobHandler:downloadBlockBlobOrAppendBlob() Ignoring range request due to invalid content range: ${err.message}`, context.contextId);
        ...
        if (rangeStart > blob.properties.contentLength) {
            throw StorageErrorFactory_1.default.getInvalidPageRange2(context.contextId, `bytes */${blob.properties.contentLength}`);
        }
