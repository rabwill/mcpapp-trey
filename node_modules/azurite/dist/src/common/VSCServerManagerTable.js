...
class VSCServerManagerTable extends VSCServerManagerBase_1.default {
    ...
    async startImpl() {
        ...
        await Telemetry_1.AzuriteTelemetryClient.TraceStartEvent("Table-VSC");
    }
    async closeImpl() {
        Telemetry_1.AzuriteTelemetryClient.TraceStopEvent("Table-VSC");
        this.server.close();
    }
    async cleanImpl() {
    ...
    async getConfiguration() {
        ...
        Telemetry_1.AzuriteTelemetryClient.init(location, !env.disableTelemetry(), env.workspaceConfiguration, true);
        // Initialize server configuration
        const config = new TableConfiguration_1.default(env.tableHost(), env.tablePort(), env.tableKeepAliveTimeout(), (0, path_1.join)(location, constants_1.DEFAULT_TABLE_LOKI_DB_PATH), (await env.debug()) === true, !env.silent(), this.accessChannelStream, undefined, env.loose(), env.skipApiVersionCheck(), env.cert(), env.key(), env.pwd(), env.oauth(), env.disableProductStyleUrl(), env.inMemoryPersistence());
        return config;
    }
}
