...
function shutdown(blobServer, queueServer, tableServer) {
    ...
    Telemetry_1.AzuriteTelemetryClient.TraceStopEvent();
...
async function main() {
    ...
    const queueConfig = new QueueConfiguration_1.default(env.queueHost(), env.queuePort(), env.queueKeepAliveTimeout(), (0, path_1.join)(location, constants_1.DEFAULT_QUEUE_LOKI_DB_PATH), (0, path_1.join)(location, constants_1.DEFAULT_QUEUE_EXTENT_LOKI_DB_PATH), constants_1.DEFAULT_QUEUE_PERSISTENCE_ARRAY, !env.silent(), undefined, env.debug() !== undefined, await env.debug(), env.loose(), env.skipApiVersionCheck(), env.cert(), env.key(), env.pwd(), env.oauth(), env.disableProductStyleUrl(), env.inMemoryPersistence());
    const tableConfig = new TableConfiguration_1.default(env.tableHost(), env.tablePort(), env.tableKeepAliveTimeout(), (0, path_1.join)(location, constants_2.DEFAULT_TABLE_LOKI_DB_PATH), env.debug() !== undefined, !env.silent(), undefined, await env.debug(), env.loose(), env.skipApiVersionCheck(), env.cert(), env.key(), env.pwd(), env.oauth(), env.disableProductStyleUrl(), env.inMemoryPersistence());
    ...
    await Telemetry_1.AzuriteTelemetryClient.TraceStartEvent();
    // Handle close event
    process
        .once("message", (msg) => {
        if (msg === "shutdown") {
